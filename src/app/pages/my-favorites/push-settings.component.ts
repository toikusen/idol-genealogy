...
@Component({
  ...
  template: `
    <div style="padding:20px;max-width:480px;">
      ...
      @if (!pushService.isSupported()) {
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;padding:12px 0;border-bottom:1px solid rgba(232,121,160,0.08);">
          <div>
            ...
            <div style="font-size:0.78rem;color:var(--text-faint-55);margin-top:3px;">
              {{ permissionLabel() }}
            </div>
          </div>
          @if (permission() === 'granted') {
            <button (click)="unsubscribe()" [disabled]="loading()"
              style="font-size:0.8rem;padding:6px 14px;border-radius:10px;border:1px solid rgba(232,121,160,0.3);background:transparent;cursor:pointer;color:var(--text-faint-75);">
              {{ loading() ? '處理中…' : '取消訂閱' }}
            </button>
          } @else {
            <button (click)="subscribe()" [disabled]="loading()"
              style="font-size:0.8rem;padding:6px 14px;border-radius:10px;border:none;background:rgba(232,121,160,1);color:white;cursor:pointer;">
              {{ loading() ? '處理中…' : '開啟通知' }}
            </button>
          }
        </div>
        ...
        @if (error()) {
          <div style="margin-top:10px;font-size:0.78rem;color:rgba(192,80,128,0.8);">{{ error() }}</div>
        }

        <div style="margin-top:16px;font-size:0.82rem;color:var(--text-faint-55);line-height:1.8;">
      ...
      <!-- iOS tip -->
      <div style="margin-top:16px;padding:14px 16px;background:rgba(147,197,253,0.08);border:1px solid rgba(147,197,253,0.25);border-radius:10px;font-size:0.82rem;color:var(--text-faint-75);line-height:1.7;">
